...
public:
	...
	{
		at_exit_table_.construct ();
	}

	static void terminate ()
	{
		while (!at_exit_table_->empty ()) {
			(*at_exit_table_->top ()) ();
			at_exit_table_->pop ();
		}
		at_exit_table_.destruct ();
	}
...
private:
	typedef std::stack <AtExitFunc, std::deque <AtExitFunc, HostAllocator <AtExitFunc> > > AtExitTable;

	static StaticallyAllocated <AtExitTable> at_exit_table_;
};
